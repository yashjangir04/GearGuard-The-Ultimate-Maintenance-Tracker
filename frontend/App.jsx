...
// --- Sub-components (Simplified for Demo) ---
...
const App = () => {
  ...
  // Business Logic: Scraping equipment when request stage is Scrap
      ...
      if (req.id === requestId) {
        ...
        if (newStage === RequestStage.SCRAP) {
          setEquipment(eqs => eqs.map(eq => 
            eq.id === req.equipmentId ? { ...eq, status: EquipmentStatus.SCRAPPED } : eq
          ));
        }
        // If moved to Repaired, mark equipment as operational
        if (newStage === RequestStage.REPAIRED) {
           setEquipment(eqs => eqs.map(eq => 
            eq.id === req.equipmentId ? { ...eq, status: EquipmentStatus.OPERATIONAL } : eq
          ));
        }
        // If moved to In Progress, mark equipment as under repair
        if (newStage === RequestStage.IN_PROGRESS) {
           setEquipment(eqs => eqs.map(eq => 
            eq.id === req.equipmentId ? { ...eq, status: EquipmentStatus.UNDER_REPAIR } : eq
          ));
        }
  ...
  const navItems = [
    ...
    { id: 'kanban', label: 'Maintenance', icon: Wrench },
    ...
    <div className="flex h-screen overflow-hidden text-slate-100 bg-slate-950">
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-slate-900 border-r border-slate-800 transition-all duration-300 flex flex-col`}>
        <div className="p-6 flex items-center gap-3">
          ...
          {sidebarOpen && <h1 className="text-xl font-bold tracking-tight">GearGuard</h1>}
        </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-slate-950">
        ...
        <header className="h-16 border-b border-slate-800 bg-slate-900/50 backdrop-blur-md px-8 flex items-center justify-between z-10">
          <div className="flex items-center gap-4">
            <h2 className="text-lg font-semibold capitalize text-slate-200">{currentView}</h2>
          ...
          <div className="flex items-center gap-4">
            <button className="p-2 text-slate-400 hover:text-slate-100 relative transition-colors">
              <Bell size={20} />
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-slate-800">
              <img src="https://picsum.photos/seed/admin/100" className="w-8 h-8 rounded-full ring-2 ring-indigo-500/20" alt="Admin" />
        ...
        <div className="flex-1 overflow-y-auto p-8 relative">
          {currentView === 'dashboard' && (
            <Dashboard 
              requests={requests} 
              equipment={equipment} 
              onViewKanban={() => setCurrentView('kanban')}
            />
          )}
          {currentView === 'kanban' && (
            <KanbanBoard 
          ...
          {currentView === 'calendar' && (
            <CalendarView 
              requests={requests.filter(r => r.type === RequestType.PREVENTIVE)}
              onDateClick={(date) => {
                setSelectedRequest({
          ...
          {currentView === 'teams' && (
            <TeamList teams={teams} technicians={technicians} />
          )}
          {currentView === 'reporting' && (
            <Reporting requests={requests} equipment={equipment} teams={teams} />
          )}
        </div>
      </main>
