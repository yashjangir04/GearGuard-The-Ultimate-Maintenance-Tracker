...
import { AlertTriangle, Clock, CheckCircle2, Zap, ArrowRight, Activity } from 'lucide-react';

const Dashboard = ({ requests, equipment, onViewKanban }) => {
  const criticalEquipmentCount = equipment.filter(e => e.health < 40 && e.status !== EquipmentStatus.SCRAPPED).length;
  const openRequests = requests.filter(r => r.stage !== RequestStage.REPAIRED && r.stage !== RequestStage.SCRAP);
  const overdueRequests = openRequests.filter(r => new Date(r.scheduledDate) < new Date());
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-2xl flex flex-col gap-4 relative overflow-hidden group hover:bg-red-500/15 transition-all">
          ...
          <div>
            <h3 className="text-4xl font-bold text-red-500">{criticalEquipmentCount} Units</h3>
            <p className="text-sm text-red-400/80 mt-1">Health &lt; 30% or High Risk</p>
          </div>
        ...
        <div className="bg-indigo-500/10 border border-indigo-500/20 p-6 rounded-2xl flex flex-col gap-4 relative overflow-hidden group hover:bg-indigo-500/15 transition-all">
          ...
          <div>
            <h3 className="text-4xl font-bold text-indigo-500">{techUtilization}% Utilized</h3>
            <p className="text-sm text-indigo-400/80 mt-1">Assign additional tasks carefully</p>
          </div>
          <div className="w-full bg-slate-800 h-2 rounded-full mt-2">
            <div className="bg-indigo-500 h-2 rounded-full" style={{ width: `${techUtilization}%` }}></div>
          </div>
        </div>
        ...
        <div className="bg-emerald-500/10 border border-emerald-500/20 p-6 rounded-2xl flex flex-col gap-4 relative overflow-hidden group hover:bg-emerald-500/15 transition-all">
          ...
          <div>
            <h3 className="text-4xl font-bold text-emerald-500">{openRequests.length} Pending</h3>
            <p className="text-sm text-emerald-400/80 mt-1">{overdueRequests.length} Overdue tasks</p>
          </div>
          <button 
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
          ...
          <button onClick={onViewKanban} className="text-sm text-indigo-400 hover:text-indigo-300">View All</button>
        </div>
              ...
              {requests.slice(0, 5).map(req => {
                ...
                return (
                  <tr key={req.id} className="hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-100">{req.subject}</td>
                    ...
                    <td className="px-6 py-4">
                      <span className="bg-slate-800 text-slate-400 px-2 py-1 rounded text-xs">{eq?.category || 'General'}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-tight ${
                        req.stage === RequestStage.NEW ? 'bg-blue-500/20 text-blue-400' :
                        req.stage === RequestStage.IN_PROGRESS ? 'bg-amber-500/20 text-amber-400' :
                        req.stage === RequestStage.REPAIRED ? 'bg-emerald-500/20 text-emerald-400' :
