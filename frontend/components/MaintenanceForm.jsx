...
import { X, Calendar, Clock, Clipboard, User, Info, CheckCircle2 } from 'lucide-react';
...
const MaintenanceForm = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    scheduledDate: request?.scheduledDate ? request.scheduledDate.split('T')[0] : new Date().toISOString().split('T')[0],
      ...
      <div className="w-full max-w-2xl h-full bg-slate-900 shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-right duration-300">
        <div className="px-8 py-6 border-b border-slate-800 flex items-center justify-between bg-slate-900/50 sticky top-0 z-10 backdrop-blur-md">
          <div className="flex items-center gap-4">
             ...
             <div>
               <h2 className="text-xl font-bold text-slate-100">{request?.id ? 'Edit Request' : 'New Maintenance Request'}</h2>
               <p className="text-xs text-slate-500 uppercase tracking-widest">{request?.id ? `Ticket #${request.id.slice(-6)}` : 'Initiate Maintenance Procedure'}</p>
             </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          ...
          <div className="flex items-center gap-2 mb-8 overflow-x-auto no-scrollbar">
            {['New', 'In Progress', 'Repaired', 'Scrap'].map((step, idx) => (
              <React.Fragment key={step}>
                <div className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap ${
                   request?.stage === step ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'bg-slate-800 text-slate-500'
                }`}>
                  {step}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
               <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Request Type</label>
               <div className="flex gap-4 p-1 bg-slate-800 rounded-xl">
                  <button 
                    onClick={() => setFormData({ ...formData, type: RequestType.CORRECTIVE })}
                    className={`flex-1 py-2 px-4 rounded-lg text-xs font-bold transition-all ${
                      formData.type === RequestType.CORRECTIVE ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    Breakdown
                  </button>
                  <button 
                    onClick={() => setFormData({ ...formData, type: RequestType.PREVENTIVE })}
                    className={`flex-1 py-2 px-4 rounded-lg text-xs font-bold transition-all ${
                      formData.type === RequestType.PREVENTIVE ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    Routine Check
                  </button>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                <Calendar size={14} className="text-indigo-400" /> Scheduled Date
              </label>
              <input 
                type="date" 
                className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-200"
                ...
                {['low', 'medium', 'high'].map(p => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-[10px] font-bold uppercase transition-all border ${
                      formData.priority === p 
                        ? p === 'high' ? 'bg-red-500/20 border-red-500 text-red-500' : p === 'medium' ? 'bg-amber-500/20 border-amber-500 text-amber-500' : 'bg-blue-500/20 border-blue-500 text-blue-500'
            ...
            <div className="col-span-2 space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Internal Notes & Instructions</label>
              <textarea 
                rows={4}
